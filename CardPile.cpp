...
Card CardPile::drawFromDiscardTop(int pileNo) {
    if (pileNo == 1) {
        if (discardPile1.empty()) {
            std::cout << "[牌堆] 弃牌堆1为空，不可抽取！" << std::endl;
            return Card();
        }
        return popCardFrom(discardPile1);
    } else if (pileNo == 2) {
        if (discardPile2.empty()) {
            std::cout << "[牌堆] 弃牌堆2为空，不可抽取！" << std::endl;
            return Card();
        }
...
// 将牌放回主牌库底部，保持传入顺序
void CardPile::putToMainBottom(const std::vector<Card>& cards) {
    mainPile.insert(mainPile.begin(), cards.begin(), cards.end());
}
bool CardPile::removeFromMain(const Card& card) {
    auto it = std::find_if(mainPile.begin(), mainPile.end(), [&](const Card& c) {
        return c.getCardName() == card.getCardName() && c.getMainType() == card.getMainType();
    });
    if (it == mainPile.end()) {
        return false;
    }
...
Card CardPile::takeCollectFromMain(CollectSubType subType) {
    auto it = std::find_if(mainPile.begin(), mainPile.end(), [&](const Card& c) {
        return c.getMainType() == CardType::COLLECT_CARD && c.getCollectSubType() == subType;
    });
    if (it == mainPile.end()) {
        return Card();
    }
