...
bool GameRule::isValidPair(const Card& c1, const Card& c2) {
    ...
    // 3. 普通对牌：同子类型有效，但排除鲨鱼、游泳者（这两类不能同子类型凑对）
    if (sub1 == sub2) {
        // 禁止：两张鲨鱼、两张游泳者凑对
        if (sub1 == PairSubType::SWIMMER || sub1 == PairSubType::SHARK) {
            return false;
        }
        // 允许：其他对牌同子类型凑对（鱼/螃蟹/船/水母/海龟/问号/龙虾）
        return true;
    }
...
void GameRule::triggerPairEffect(const Card& pairCard, Player& currPlayer) {
    ...
    case PairSubType::CRAB: {
        // 螃蟹牌：从弃牌堆抽1张（仅剩1张不可抽取规则）
        Card drawnCard;

        // 优先尝试抽取弃牌堆1
        if (!gamePile.isDiscardPileEmpty(1)) {
            drawnCard = gamePile.drawFromDiscardTop(1);
        }

        // 弃牌堆1抽取失败，尝试抽取弃牌堆2
        if (drawnCard.getCardName() == "空" && !gamePile.isDiscardPileEmpty(2)) {
            drawnCard = gamePile.drawFromDiscardTop(2);
        }
...
void GameRule::triggerEventEffect(Player& currPlayer) {
    ...
    case EventSubType::MERMAID_E: {
        // 美人鱼事件：获得1张美人鱼牌（添加到收集牌）
        Card mermaidCard(CardType::COLLECT_CARD, CollectSubType::MERMAID, CardColor::RED, "美人鱼牌" + std::to_string(currPlayer.getMermaidCount() + 1), 0);
        currPlayer.addCollectCard(mermaidCard); // 直接添加到收集牌
        currPlayer.addMermaidCard(); // 增加美人鱼计数
...
void GameRule::nextPlayer() {
    ...
    } while (players[currentPlayerIndex].isPlayerSkipTurn());
    // ✅ 新增：新玩家回合开始，重置抽牌状态
...
void GameRule::prepareNextRound(int nextStartIndex) {
    ...
    for (auto& p : players) {
        ...
        p.addHandCards(initHand);
        p.resetTurnState();
        std::cout << "[" << p.getPlayerName() << "] 新一轮获得起始手牌2张！" << std::endl;
    }
...
void GameRule::roundEnd(bool isAllIn, int allInStarterIndex) {
    ...
    for (int i = 0; i < playerNum; ++i) {
        ...
        colorBonuses[i] = calcColorBonus(players[i]);
    }
    ...
    for (int i = 0; i < playerNum; ++i) {
        ...
        if (!isAllIn) {
            ...
            // 放手一搏成功：发起者拿基础分+颜色奖励；其他玩家仅拿颜色奖励。
            ...
            // 放手一搏失败：发起者仅拿颜色奖励；其他玩家拿基础分+颜色奖励。
            totalRound = (i == starterIndex) ? colorBonuses[i] : (baseScores[i] + colorBonuses[i]);
...
int GameRule::getMaxColorCount() const {
    ...
    for (const auto& player : players) {
        std::unordered_map<CardColor, int> colorCount;
        // 统计玩家手牌+收集牌的颜色数量
        for (const auto& card : player.getHandCards()) colorCount[card.getColor()]++;
        for (const auto& card : player.getCollectCards()) colorCount[card.getColor()]++;
        // 更新全场最大值
